...
class ${ModelName}ViewSet(DataScopeFilterMixin, AuditOwnerPopulateMixin, SoftDeleteMixin, ActionSerializerMixin, viewsets.ModelViewSet):
    ...
    queryset = ${ModelName}.objects.select_related('owner_organization', 'created_by', 'updated_by').all().order_by('-id')
    ...
    # 默认
    serializer_class = ${ModelName}DetailSerializer

    # 按动作切换
    list_serializer_class = ${ModelName}ListSerializer
    retrieve_serializer_class = ${ModelName}DetailSerializer
    create_serializer_class = ${ModelName}CreateSerializer
    update_serializer_class = ${ModelName}UpdateSerializer
    partial_update_serializer_class = ${ModelName}UpdateSerializer
